...
export default {
  setup() {
    ...
    const endTime = new Date().getTime() + 88400000;
    const countdown = () => {
      const nowTime = new Date().getTime();
      const timeDiff = endTime - nowTime;
      time.DD = parseInt(timeDiff / 1000 / 60 / 60 / 24);
      time.HH = parseInt(timeDiff / 1000 / 60 / 60 % 24);
      time.MM = parseInt(timeDiff / 1000 / 60 % 60);
      time.SS = parseInt(timeDiff / 1000 % 60);
      time.MS = parseInt(timeDiff % 1000 / 100);
    }
    onMounted(() => {
      countdown();
